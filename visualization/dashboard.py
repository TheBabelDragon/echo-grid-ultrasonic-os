#!/usr/bin/env python3
"""Echo Grid — Kalman multi-track CSI display."""
...
TRACK_COLORS = ["#ff4d6d", "#4cc9f0", "#f4a261", "#a0e8af", "#c77dff", "#ffe066", "#ff85a1", "#90e0ef"]
STATE_STYLE = {"idle": "--", "move": "-", "surge": "-"}


class LiveDashboard:
    def __init__(self, size=16, body_port=None, csi_port=None, drive=False, closed_loop=True, demo=True):
        self.osys = EchoGridOS(size=size, body_port=body_port, csi_port=csi_port)
        self.drive, self.closed_loop, self.demo, self.size = drive, closed_loop, demo, size
        ...
        self.ax_phi.set_title("Phase φ + Kalman tracks")
        ...
        for color in TRACK_COLORS:
            ring = Circle((0.5, 0.5), 0.06, fill=False, edgecolor=color, linewidth=2, alpha=0, zorder=6)
            dot, = self.ax_phi.plot([0.5], [0.5], "o", color=color, markersize=8,
        ...
        self.energy_bg = FancyBboxPatch((0.12, 0.72), 0.76, 0.12, boxstyle="round,pad=0.01,rounding_size=0.02",
        ...
        self.energy_label = self.ax_csi.text(0.5, 0.88, "energy  0.00", ha="center", va="bottom",
        ...
        if not bits:
            bits.append("soft")
        self.fig.suptitle(f"Echo Grid  ·  mode={'+'.join(bits)}  ·  Kalman+Hungarian", fontsize=12)
    ...
    def update(self, _frame):
        if self.demo:
            ...
            self.osys.touch(0.5 + 0.25 * np.sin(t * 0.5), 0.5 + 0.25 * np.cos(t * 0.37), strength=0.15)
        ...
        self.im_phi.set_clim(-max(0.35, min(2, pmax * 1.15)), max(0.35, min(2, pmax * 1.15)))
        ...
        for i in range(len(TRACK_COLORS)):
            ...
            if i < len(tracks):
                tr = tracks[i]
                x, y = tr.pos
                alpha = min(1.0, 0.25 + tr.confidence * 0.75)
                ring.center = (x, y)
                ring.set_radius(0.03 + 0.12 * tr.energy)
                ring.set_alpha(alpha)
                ring.set_linestyle(STATE_STYLE.get(tr.state, "-"))
                ring.set_linewidth(1.4 + 2.2 * tr.confidence)
                dot.set_data([x], [y])
                dot.set_alpha(alpha)
        ...
        self.status.set_text(
            f"entropy={self.osys.field.entropy:.3f}  csi={e:.3f}  "
...
def main():
    ...
    a = p.parse_args()
    print(f"[dashboard] backend={_BACKEND} csi={a.csi}")
    LiveDashboard(a.size, a.body, a.csi, a.drive, not a.no_loop, not a.no_demo).run()
